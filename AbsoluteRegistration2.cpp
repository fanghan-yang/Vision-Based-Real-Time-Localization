...
cv::Scalar color_orange = cv::Scalar(0, 127, 255);
...
void* Optical_Flow(void* param) 
{
	...
	for (int iFile = 0; iFile < vFilename.size(); iFile++)
	{
		...
		DLT_ExtrinsicCameraParamEstimationWRansac_EPNP_mem_abs(cX, cx, K, P, 5, 10, vInlier);
		...
		/////////////////////////////// recompute P /////////////////////////////////
		/////////////////////////////////////////////////////////////////////////////
		CvMat *Xin = cvCreateMat(vInlier.size(), 3, CV_32FC1);
		CvMat *xin = cvCreateMat(vInlier.size(), 2, CV_32FC1);

		for (int iInlier = 0; iInlier < vInlier.size(); iInlier++)
		{
			cvSetReal2D(Xin, iInlier, 0, cvGetReal2D(cX, vInlier[iInlier], 0));
			cvSetReal2D(Xin, iInlier, 1, cvGetReal2D(cX, vInlier[iInlier], 1));
			cvSetReal2D(Xin, iInlier, 2, cvGetReal2D(cX, vInlier[iInlier], 2));
		...
		for (int i = 0; i < 10; i++) {
			...
			CvMat *X4 = cvCreateMat(4, vInlier.size(), CV_32FC1);
			CvMat *x4 = cvCreateMat(3, vInlier.size(), CV_32FC1);
			for (int j = 0; j < vInlier.size(); j++) {
				cvSetReal2D(X4, 0, j, cvGetReal2D(Xin, j, 0));
				cvSetReal2D(X4, 1, j, cvGetReal2D(Xin, j, 1));
				cvSetReal2D(X4, 2, j, cvGetReal2D(Xin, j, 2));
				cvSetReal2D(X4, 3, j, 1);
			}
			cvMatMul(P, X4, x4);
		
			for (int j = 0; j < vInlier.size(); j++) {
				cvSetReal2D(xin, j, 0, cvGetReal2D(x4, 0, j)/cvGetReal2D(x4, 2, j));
				cvSetReal2D(xin, j, 1, cvGetReal2D(x4, 1, j)/cvGetReal2D(x4, 2, j));
			}
		...
		/////////////////////////////////////////////////////////////////
		/////////////////////	compute median and refinement
		/////////////////////////////////////////////////////////////////
		vector<double> reprojError0;

		CvMat *R = cvCreateMat(3,3,CV_32FC1);
		CvMat *C = cvCreateMat(3,1,CV_32FC1);
		GetCameraParameter(P, K, R, C);

		vector<double> vu0, vv0, vu10, vv10;
		...
		for (int i = 0; i < vCorr.size(); i++)
		{
			...
			cvMatMul(P, X4, x4);
			vu0.push_back(cvGetReal2D(x4, 0, 0)/cvGetReal2D(x4, 2, 0));
			vv0.push_back(cvGetReal2D(x4, 1, 0)/cvGetReal2D(x4, 2, 0));
		...
		Distortion_Radial(vu0, vv0);
		Distortion_Radial(vu10, vv10);
		...
		for (int i = 0; i < vInlier.size(); i++)
		{
			...
			reprojError0.push_back(sqrt((p1.x-p2.x)*(p1.x-p2.x) + (p1.y-p2.y)*(p1.y-p2.y)));
		}		

		cvReleaseMat(&C);
		cvReleaseMat(&R);

		sort(reprojError0.begin(), reprojError0.end());

		double median0;
		if (reprojError0.size() % 2 == 0)
			median0 = (reprojError0[reprojError0.size() / 2 - 1] + reprojError0[reprojError0.size() / 2]) / 2;
		else 
			median0 = reprojError0[reprojError0.size() / 2];
		
		vector<int> vInlier2;
		for (int iPoint = 0; iPoint < vInlier.size(); iPoint++)
		{
			cv::Point2f p1, p2;
			p1.x = vu0[vInlier[iPoint]];
			p1.y = vv0[vInlier[iPoint]];

			p2.x = vu10[vInlier[iPoint]];
			p2.y = vv10[vInlier[iPoint]];
	
			if (sqrt((p1.x-p2.x)*(p1.x-p2.x) + (p1.y-p2.y)*(p1.y-p2.y)) < median0)
				vInlier2.push_back(vInlier[iPoint]);
		}
		
		// refinement
		CvMat *cX_ransac = cvCreateMat(vInlier2.size(), 3, CV_32FC1);
		CvMat *cx_ransac = cvCreateMat(vInlier2.size(), 2, CV_32FC1);

		for (int iPoint = 0; iPoint < vInlier2.size(); iPoint++)
		{
			cvSetReal2D(cX_ransac, iPoint, 0, vCorr[vInlier2[iPoint]].x);
			cvSetReal2D(cX_ransac, iPoint, 1, vCorr[vInlier2[iPoint]].y);
			cvSetReal2D(cX_ransac, iPoint, 2, vCorr[vInlier2[iPoint]].z);

			cvSetReal2D(cx_ransac, iPoint, 0, vCorr[vInlier2[iPoint]].u);
			cvSetReal2D(cx_ransac, iPoint, 1, vCorr[vInlier2[iPoint]].v);
		}
		
		gettimeofday(&t1,NULL);
		AbsoluteCameraPoseRefinement_Jacobian(cX_ransac, cx_ransac, P, K, 5);
		gettimeofday(&t2,NULL);
		...
		*/
		...
		{			
			if (vInlier.size() >= 20)
			{
				vector<double> reprojError; // debug
				...
				/*
				// plot back reprojection of features before pnp
				for (int i = 0; i < vCorr.size(); i++)
				{
					cv::Point2f p1, p2;
					p1.x = vu[i];
					...
					line(img_debug, p1, p2, color_orange, 1);
				...
				*/
				...
				for (int i = 0; i < vInlier.size(); i++)
				{
					...
					circle(img_debug, p1, 2, color_yellow, -1);
					line(img_debug, p1, p2, color_orange, 1);

					double error_temp = sqrt((p1.x-p2.x)*(p1.x-p2.x) + (p1.y-p2.y)*(p1.y-p2.y));
					if ( error_temp >= 0 && error_temp < 100) {
						reprojError.push_back(error_temp);
					}
				}		
				...
				// reprojection error
				double sum = std::accumulate(reprojError.begin(), reprojError.end(), 0.0);
				double mean = sum / reprojError.size();

				double sq_sum = inner_product(reprojError.begin(), reprojError.end(), reprojError.begin(), 0.0);
				double stdev = sqrt(sq_sum / reprojError.size() - mean * mean);

				sort(reprojError.begin(), reprojError.end());

				double median;
				if (reprojError.size() % 2 == 0)
					median = (reprojError[reprojError.size() / 2 - 1] + reprojError[reprojError.size() / 2]) / 2;
				else 
					median = reprojError[reprojError.size() / 2];

				// show error result
				string myStr1;
				ostringstream myTemp1;
				myTemp1 << mean;
				myStr1 = myTemp1.str();
				cv::Point textPos1;
				textPos1.x = 600;
				textPos1.y = 35;
				putText(img_debug, myStr1, textPos1, 2, 0.8, color_red);

				string myStr2;
				ostringstream myTemp2;
				myTemp2 << median;
				myStr2 = myTemp2.str();
				cv::Point textPos2;
				textPos2.x = 600;
				textPos2.y = 70;
				putText(img_debug, myStr2, textPos2, 2, 0.8, color_red);

				string myStr3;
				ostringstream myTemp3;
				myTemp3 << stdev;
				myStr3 = myTemp3.str();
				cv::Point textPos3;
				textPos3.x = 600;
				textPos3.y = 105;
				putText(img_debug, myStr3, textPos3, 2, 0.8, color_red);

				// show runtime
				string myStr4;
				ostringstream myTemp4;
				myTemp4 << t_opt;
				myStr4 = myTemp4.str();
				cv::Point textPos4;
				textPos4.x = 35;
				textPos4.y = 390;
				putText(img_debug, myStr4, textPos4, 2, 0.8, color_red);

				string myStr5;
				ostringstream myTemp5;
				myTemp5 << t_pnp;
				myStr5 = myTemp5.str();
				cv::Point textPos5;
				textPos5.x = 35;
				textPos5.y = 420;
				putText(img_debug, myStr5, textPos5, 2, 0.8, color_red);
			}
...
{	
	...
	while(true) 
	{
		...
		// DLT_ExtrinsicCameraParamEstimationWRansac_EPNP_mem_abs(cX, cx, K, P, 10, 100, vInlier);
		DLT_ExtrinsicCameraParamEstimationWRansac_EPNP_mem_abs(cX, cx, K, P, 10, 100, vInlier);
